...
function WelcomePage() {
  ...
  useEffect(() => {
    const numDuration = 1500;
    const numInterval = 20;
    const numStep = (numInterval / numDuration) * 100;
    const timer = setInterval(() => {
      setNumProgress(prev => Math.min(prev + numStep, 100));
    }, numInterval);
  ...
  return (
    <>
      ...
      <div className="v2-welcome-content" style={{
        ...
        <span style={{ fontSize: '0.75rem', color: 'var(--v2-text-muted)', marginTop: '0.5rem' }}>entering in 1.5s…</span>
      </div>
    </>
  );
}
