...
const CommentBody = ({
  comment, loadingCommentsState, onAnalyze, onStartReply, onDelete, onEdit,
  boolIsLoggedIn, boolIsAdmin, numCurrentUserId, strRequesterName,
}) => {
  ...
  const [strEditContentState, setStrEditContentState] = useState(comment.strContent);

  const boolCanEdit = !!onEdit && (boolIsAdmin || (numCurrentUserId != null && comment.user_id === numCurrentUserId));

  const handleStartEdit = () => {
    setStrEditContentState(comment.strContent);
  ...
  const handleSaveEdit = async () => {
    if (!strEditContentState.trim()) return;
    await onEdit(comment.id, strEditContentState.trim());
    setBoolIsEditingState(false);
  };
    ...
    <>
      ...
      {boolIsEditingState ? (
        <div style={{ marginBottom: '0.6rem' }}>
          <textarea
            ...
            style={{ minHeight: '60px', resize: 'vertical', width: '100%', fontSize: '0.875rem' }}
            autoFocus
          />
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.4rem' }}>
            ...
            <button
              ...
              style={{ padding: '4px 12px', fontSize: '0.75rem', border: 'none', background: 'transparent', color: 'var(--cr-text-muted)' }}
            >
              Cancel
            </button>
          </div>
      ...
      <div style={{ display: 'grid', gap: '0.6rem', marginBottom: '0.8rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.2rem' }}>
          ...
          {boolCanEdit && !boolIsEditingState && (
            <button
              className="verisphere-btn-outline"
...
CommentBody.propTypes = {
  ...
  numCurrentUserId: PropTypes.number,
