...
describe('useAuth', () => {
  ...
  it('handleLogin sets token and fetches profile', async () => {
    loginUser.mockResolvedValueOnce({ access: 'new_tok', refresh: 'new_ref' });
  ...
  it('handleLogout clears token and user', async () => {
    loginUser.mockResolvedValueOnce({ access: 'tok', refresh: 'ref' });
    fetchUserProfile.mockResolvedValueOnce({ username: 'alice' });
  ...
  it('stores access and refresh tokens in sessionStorage on login', async () => {
    loginUser.mockResolvedValueOnce({ access: 'stored_tok', refresh: 'stored_ref' });
    ...
    await waitFor(() => {
      ...
      expect(sessionStorage.getItem('refresh_token')).toBe('stored_ref');
    });
  });
});
