...
describe('AuthModal integration', () => {
  ...
  it('calls handleRegister then returns to login with an activation message', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Welcome Back')).toBeDefined();
      expect(screen.getByText('Check your email for an activation link.')).toBeDefined();
    });
    expect(onClose).not.toHaveBeenCalled();
  });

  it('walks the forgot-password flow', async () => {
    const handleForgotPassword = vi.fn(() => Promise.resolve());

    renderModal({
      useAuthHook: { handleLogin: vi.fn(), handleRegister: vi.fn(), handleForgotPassword },
    });
    ...
    await waitFor(() => {
      expect(handleForgotPassword).toHaveBeenCalledWith('alice@test.com');
      expect(screen.getByText('Welcome Back')).toBeDefined();
      expect(screen.getByText('If that account exists, a reset link has been sent.')).toBeDefined();
    });
  });
